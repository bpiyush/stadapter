...
def main():
  ...
  parser.add_argument('--wandb_entity', type=str, default="bpiyush",
      help='wandb entity name')
  parser.add_argument('--wandb_project', type=str, default="time-antonyms",
      help='wandb project name')
  parser.add_argument('--wandb_run_name', type=str, default="clip_vitb_p16_meanpool-ssv2",
      help='wandb run name')
  ...
  if args.use_wandb and dist.get_rank() == 0:
    wandb.init(
        ...
        dir="/work/piyush/experiments/stadapter/",
        config=vars(args)
    )
  ...
  def evaluate(log_stats=None):
    ...
    for data, labels in metric_logger.log_every(dataloader_val, 100, header):
      ...
      with torch.cuda.amp.autocast():
        ...
        if args.use_wandb and dist.get_rank() == 0:
            preds = scores.topk(1, dim=1)[1].cpu().numpy()
    ...
    if args.use_wandb and dist.get_rank() == 0:
        wandb_metrics = {'val_acc1': metric_logger.acc1.global_avg, 
                         'val_acc5': metric_logger.acc5.global_avg}
        wandb.log(wandb_metrics)
        
        # Log confusion matrix if we have enough predictions
        if len(all_preds) > 0 and len(all_labels) > 0:
            try:
                wandb.log({"confusion_matrix": wandb.plot.confusion_matrix(
                    ...
                    y_true=all_labels, 
                    preds=all_preds,
                    class_names=list(range(DATASETS[args.dataset]['NUM_CLASSES']))
                )})
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    for step, (data, labels) in enumerate(metric_logger.log_every(dataloader_train, args.print_freq, header)):
      ...
      # Log metrics to wandb at regular intervals
      if args.use_wandb and dist.get_rank() == 0 and step % args.print_freq == 0:
          wandb.log({
              "train_loss": loss.item(),
              "train_acc1": acc1,
              "train_acc5": acc5,
              "learning_rate": optimizer.param_groups[0]['lr'],
              "epoch": epoch + step / len(dataloader_train),
              "step": epoch * len(dataloader_train) + step
          })
    ...
    if args.use_wandb and dist.get_rank() == 0:
        wandb.log({
            "epoch": epoch,
            "epoch_train_loss": metric_logger.loss.global_avg,
            "epoch_train_acc1": metric_logger.acc1.global_avg,
            "epoch_train_acc5": metric_logger.acc5.global_avg,
            "epoch_learning_rate": metric_logger.lr.global_avg
        })
